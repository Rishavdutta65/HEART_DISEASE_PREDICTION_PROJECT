...
def generate_medical_pdf(data, static_dir):
    ...
    patient_table_data = [
        ["Age", f"{inp.get('age', 'N/A')} yrs", "Sex", "Male" if str(inp.get('sex')) == '1' else "Female"],
        ["Chest Pain", cp_map.get(int(inp.get('cp', 0)), "N/A"), "Resting BP", f"{inp.get('trestbps', 'N/A')} mmHg"],
        ["Cholesterol", f"{inp.get('chol', 'N/A')} mg/dL", "Fasting Sugar", fbs_map.get(int(inp.get('fbs', 0)), "N/A")],
        ["Resting ECG", str(inp.get('restecg', 'N/A')), "Max Heart Rate", f"{inp.get('thalach', 'N/A')} bpm"],
        ["Exercise Angina", exang_map.get(int(inp.get('exang', 0)), "N/A"), "ST Depression", str(inp.get('oldpeak', 'N/A'))],
        ["Slope", str(inp.get('slope', 'N/A')), "Major Vessels", str(inp.get('ca', 'N/A'))],
        ["Thalassemia", str(inp.get('thal', 'N/A')), "", ""]
    ]
    ...
    if pct >= 70:
        ...
        text_color = colors.HexColor('#f59e0b')
    else:
        risk_level = "LOW RISK"
    ...
    if pct >= 70:
        rec_text = """
        • <b>Immediate Action:</b> Consult a cardiologist for a comprehensive cardiac evaluation.<br/>
        • <b>Monitoring:</b> Monitor symptoms such as chest pain, shortness of breath, or dizziness closely.<br/>
        • <b>Tests:</b> Discuss the need for advanced diagnostics (e.g., Angiogram, Stress Echo) with your doctor.<br/>
        • <b>Lifestyle:</b> Strictly adhere to any prescribed medication and avoid strenuous exertion without medical clearance.
        """
    elif pct >= 40:
        rec_text = """
        • <b>Medical Action:</b> Schedule a routine check-up with your primary care physician to discuss these results.<br/>
        • <b>Monitoring:</b> Regularly monitor blood pressure and cholesterol levels.<br/>
        • <b>Diet:</b> Adopt a heart-healthy diet low in saturated fats and sodium.<br/>
        • <b>Lifestyle:</b> Engage in moderate physical activity as recommended by a healthcare provider.
        """
    else:
        rec_text = """
        • <b>Medical Action:</b> Continue with routine annual check-ups.<br/>
        • <b>Lifestyle:</b> Maintain your current healthy habits.<br/>
        • <b>Diet & Exercise:</b> Ensure a balanced diet and regular aerobic exercise (150 mins/week) to preserve cardiovascular health.<br/>
        • <b>Stress Management:</b> Practice adequate sleep hygiene and stress-reduction techniques.
        """
